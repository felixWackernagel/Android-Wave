...
import net.processone.wave.api.ClientWave.JSONRpcHandler;
...
public class OneWaveAPI {
	...
	private static final String TAG = OneWaveAPI.class.getSimpleName();

	private static final String DOMAIN = "http://waveinabox.net/";
	...
	public OneWaveAPI(final ClientSettings settings) {
		...
		clientWave = new ClientWave(new JSONRpcHandler() {
			...
			public String request(String jsonBody) throws Exception {
				String rpcURL = settings.getRpcUrl();
				Log.d(TAG, "Client Wave request to " + rpcURL);
				return oauth.send(rpcURL, JSON_MIME_TYPE, jsonBody).readBodyAsString();
			}
		});
